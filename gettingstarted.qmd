...
-   Go into *package mode* and type `status` (or just `st`) to see if you have any packages installed. (probably not if you have a fresh install).
:::

::: {#q:modes .callout-note collapse="true" icon="false"}
## Solution

```         
# Go into help mode and read the docs about the sine function
?sin

# Step into shell mode with ; and change directory. 
; cd SomeDirectory

# Go into package mode and check the status of installed packages in the env
] st
```
:::
...
The package manager is most easily accessed by typing `]` to get into Pkg mode. The prompt changes to `(@v1.12) pkg>` which means that you are in package mode in the default environment v.1.12 (if you are using Julia 1.12, more on environments later).
...
-   `import` does not expose exported function directly and requires a call to function to be prefixed with the package name, e.g. `PkgName.myfunc(3)`. This is closer to Python's namespace convention, but namespaces are less essential in Julia due to multiple dispatch and Julia's clever handling of function (method) name collisions.
...
-   Packages are by default `add`:ed to the default environment (e.g. v1.12 if you use Julia 1.12). However, it is common to have several projects ongoing at the same time, and different projects needs different packages. Here is where *environments* are very useful.

-   A Julia **environment** is basically a list of all the packages used in a project, potentially including detailed information about which version of the packages that are to be used in the project. An environment is created by going to a directory and typing `activate .` in the Pkg prompt. The dot means *here* (in the directory structure; alternatively, the dot can be replaced by a path where you want the environment to live). The activate command creates two files:

    -   [Project.toml](Project.toml) - This is a list of the packages added to the project (so called direct dependencies), and their compatibility. Each package is identified by a unique ID. The link goes to the Project.toml used for this tutorial.

    -   [Manifest.toml](Manifest.toml) - This contains the complete dependency structure of the added packages (since your added packages will often depend on other packages, so called indirect dependencies). The manifest.toml file can be used to `instantiate` all packages and their dependencies to the exact versions used in a particular project, so that code can be exactly reproduced with a frozen state of dependencies.
...
```         
# add Distributions.jl in package manager
] add Distributions

# Load Distributions.jl
julia> using Distributions

# Set dist to a normal distribution object
dist = Normal(2,3)
...
## ![](/misc/vscodeicon.png){width="25"} VS code - Rstudio/Positron for Julia
...
`shift + enter` or `alt + enter` on a line in the editor starts the Julia REPL automatically. However, if you close the REPL, you have to use the command [Julia: Restart REPL]{.underline} in the Command palette to start a new one.

::: callout-tip
## Disable inline results
...
::: callout-tip
## Making Julia in VS Code more similar to RStudio

To make VS Code a little more like the Panes view in RStudio, you may want to search for the setting (press `ctrl+shift+p`): `Terminal Default Location` and change its value to `editor`. This will make the Julia REPL appears in the editor as a tab, which you can then drag to a new location.
:::
